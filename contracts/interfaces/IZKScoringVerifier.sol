...
interface IZKScoringVerifier {
    ...
    // Core Functions
    function verifyScoringProof(bytes calldata proof, bytes32[] calldata publicInputs) external view returns (bool);

    function verifyScoringProofWithMetadata(ScoringProofData calldata proofData)
        external
        view
        returns (ScoringVerificationResult memory);

    function batchVerifyScoringProofs(ScoringProofData[] calldata proofs)
        external
        view
        returns (ScoringVerificationResult[] memory);
    ...
    function getScoringVerificationStats()
        external
        view
        returns (uint256 totalVerifications, uint256 successfulVerifications, uint256 failedVerifications);
    function getScoringProofComplexity(bytes calldata proof) external view returns (uint256);

    // Events
    event ScoringProofVerified(address indexed verifier, bytes32 indexed modelHash, bool success, uint256 gasUsed);
